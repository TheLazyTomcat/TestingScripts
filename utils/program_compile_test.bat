...
FOR /F "usebackq tokens=1* delims=\" %%f IN ("%path_base%build_modes_fpc.txt") DO (
...
SET "file_log=%path_base%log.txt"
...
FOR %%f IN (%file_list%) DO (
  ...
  IF /I "%%~xf"==".lpi" (
    ...
    FOR %%a IN (!current_modes!) DO (
      SET /A fpc_build_mode_count+=1
    )
